...
class PatternMatchingWithMismatches:
    ...
    def find_occurrences(self, k, text, pattern):
        ...
        for i in range(n - m + 1):
            # Compare hashes for early match detection
            if self.are_equal(text_hash, pattern_hash, i, 0):
                occurrences.append(i)  # Use 0-based indexing
            else:
                # If hashes are different, count mismatches
    ...
    def process_multiple_inputs(self):
        results = []
        print("Enter 5 lines of input in the format: k text pattern")
        for _ in range(5):
