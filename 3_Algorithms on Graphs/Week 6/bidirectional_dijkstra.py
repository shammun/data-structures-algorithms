...
def bidirectional_dijkstra(graph, start, end):
    ...
    prev_backward = {node: None for node in graph}
    dist_backward[end] = 0

    # Priority queues for forwand and backward passes
    pq_forward = [(0, start)]
    pq_backward = [(0, end)]
...
def main():
    ...
    if start_node not in graph or end_node not in graph:
        print(f"Error: start node {start_node} or end node {end_node} is not in the graph.")
        return

    min_distance, forward_path, backward_path = bidirectional_dijkstra(graph, start_node, end_node)

    if min_distance == float('inf'):
        print(f"No path found from {start_node} to {end_node}.")
    else:
        # Remove meet_node from forward_path to prevent duplication
        #if forward_path and backward_path and forward_path[-1] == backward_path[-1]:
        #    forward_path.pop()
        # full_path = forward_path + "  Extra Zone  " + backward_path
